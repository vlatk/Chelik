...
exports.run = async (robot, interaction)=>{
    ...
    var tag = interaction.options.data[0].options[0].value
      await interaction.reply({ embeds: [embed] });
        ...
        case 'player':
        request({url: `https://api.brawlstars.com/v1/players/%23${tag.replace(/[^a-zё0-9]/gi, '')}/battlelog`, headers: {"Authorization": "bearer token", "Content-Type": "application/json"}}, (err, res, body) => {
          if(err) return console.error(err);
          var result1 = JSON.parse(body);
          if(result1.reason) return interaction.editReply({ embeds: [embed2] });
        ...
        request({url: `https://api.brawlstars.com/v1/players/%23${tag.replace(/[^a-zё0-9]/gi, '')}`, headers: {"Authorization": "bearer token", "Content-Type": "application/json"}}, (err, res, body) => {
          if(err) return console.error(err);
      ...
      if(result.club.tag == undefined) clubtag = '#000000000';
          if(result.reason) return interaction.editReply({ embeds: [embed3] });
         ...
         interaction.editReply({ embeds: [embed8] })
        })})
      break;
      case 'club':
    request({url: `https://api.brawlstars.com/v1/clubs/%23${tag.replace(/[^a-zё0-9]/gi, '')}`, headers: {"Authorization": "bearer token", "Content-Type": "application/json"}}, (err, res, body) => {
      if(err) return console.error(err);
      var result2 = JSON.parse(body);
      if(result2.reason) return interaction.editReply({ embeds: [embed4] });
      var clubb = ''
     ...
     interaction.editReply({ embeds: [embed9] })
  })
     break;
}}
