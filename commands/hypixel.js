...
if(args[0]){           
    request({url: `https://api.mojang.com/users/profiles/minecraft/${args[0]}`, headers: {"Content-Type": "application/json"}}, (err, res, body) => {
                if(err) return console.error(err);
                try{var result1 = JSON.parse(body);}
                catch(err){  return mess.channel.send({ embeds: [embed2] })}
            
                var uuid = result1.id
                if(!result1.name) uuid = args[0]
            request({url: `https://api.hypixel.net/player?uuid=${uuid}`, headers: {"API-Key": "hypixel api-key", "Content-Type": "application/json"}}, (err, res, body) => {
                ...
                catch(err){  return mess.channel.send({ embeds: [embed3] })}
