...
exports.run = async (robot, mess, args)=>{
  ...
  var embed4= new Discord.MessageEmbed({title: "Статистика Geometry Dash", description: `Неправильное использование команды! \n Использование: \n
   ...
   case 'player':
    ...
    request({url: `https://gdbrowser.com/api/profile/${args[1]}`, headers: {"Content-Type": "application/json"}}, (err, res, body) => {
      ...
      if(result == -1) return botmsg.edit({ embeds: [embed2] });
  ...
  case 'level':
    request({url: `https://gdbrowser.com/api/level/${args[1]}`, headers: {"Content-Type": "application/json"}}, (err, res, body) => {
      if(err) return console.error(err);
      var result = JSON.parse(body);
      if(result == -1) return botmsg.edit({ embeds: [embed3] });
    ...
    var authorr = ''
      ...
      ${desc}`}).setThumbnail(`https://gdbrowser.com/assets/difficulties/${result.difficultyFace}.png`).setColor('RANDOM').addFields(
        ...
        **Версия уровня:** \`${result.version}\`
        ...
        {name: "_ _", value: "_ _"},
        { name: '▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ', value: '_ _' }
      ).setFooter(`Автор: ${result.author} • id: ${result.accountID} • ~gdstats player ${result.accountID}`, `https://gdbrowser.com/icon/icon?form=cube&icon=${result1.icon}&col1=${result1.col1}&col2=${result1.col2}&glow=${result1.glow}`)
      botmsg.edit({ embeds: [embed6] })
  })})
  break;
  /*case "search":
